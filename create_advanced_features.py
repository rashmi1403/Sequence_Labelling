...
for utterance in utterances:
    ...
    if(dict_uttr[2] != None and dict_uttr[3] != None):
        ...
        for k in range(len(pos_list)):
            dict_pos = {}
            dict_pos = pos_list[k]
            intermed_token = "TRIGRAM_"
            intermed_pos   = "TRIGRAM_POS_"
            #print(dict_pos[0] + " " + dict_pos[1])
            if((k-1) >= 0):
                dict_pos_k1 = {}
                dict_pos_k1 = pos_list[k-1]
                if(dict_pos_k1[0] == ":"):
                    intermed_token += "COLON_"
                elif(dict_pos_k1[0] == "\\"):
                    intermed_token += "BACKSLASH_"
                else:
                    intermed_token += dict_pos_k1[0] + "_"

                if(dict_pos_k1[1] == ":"):
                    intermed_pos += "COLON_"
                elif(dict_pos_k1[1] == "\\"):
                    intermed_pos += "BACKSLASH_"
                else:
                    intermed_pos += dict_pos_k1[1] + "_"
                    
            if(dict_pos[0] == ":"):
                intermed_token += "COLON"
            elif(dict_pos[0] == "\\"):
                intermed_token += "BACKSLASH"
            else:
                intermed_token += dict_pos[0]

            if(dict_pos[1] == ":"):
                intermed_pos += "COLON"
            elif(dict_pos[1] == "\\"):
                intermed_pos += "BACKSLASH"
            else:
                intermed_pos += dict_pos[1]     
            
            if((k+1) < len(pos_list)):
                dict_pos_k2 = {}
                dict_pos_k2 = pos_list[k+1]
                if(dict_pos_k2[0] == ":"):
                    intermed_token += "_COLON"
                elif(dict_pos_k2[0] == "\\"):
                    intermed_token += "_BACKSLASH"
                else:
                    intermed_token += "_" + dict_pos_k2[0] 

                if(dict_pos_k2[1] == ":"):
                    intermed_pos += "_COLON"
                elif(dict_pos_k2[1] == "\\"):
                    intermed_pos += "_BACKSLASH"
                else:
                    intermed_pos += "_" + dict_pos_k2[1]

            
            output_str += intermed_token + "\t"
            output_str += intermed_pos + "\t"
